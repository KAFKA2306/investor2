...
from scripts import build_ark_source_health as MODULE
...
class ArkSourceHealthTest(unittest.TestCase):
    ...
    def test_blocked_and_deferred_sources_are_not_fetched(self):
        ...
        self.assertEqual(
            result["status_counts"]["blocked_external_evidence"],
            1,
        )
    ...
    def test_ready_source_records_hash_without_copying_payload(self):
        ...
        payload = {"coverage": {"count": 3}, "dataset": "x"}
        raw = b'{"coverage":{"count":3},"dataset":"x"}'
        with patch.object(MODULE, "fetch_json", return_value=(raw, payload)):
            result = MODULE.build_health(catalog)
