...
def verified_probe(repository, evidence_url=None):
    return {
        "exists": True,
        "scheduled": True,
        "latest_passed": True,
...
class ArkRepositoryCoverageTest(unittest.TestCase):
    ...
    def test_great_acceleration_does_not_require_a_dedicated_repo(self):
        result = self.build()
        row = next(r for r in result["records"] if r["theme"] == "The Great Acceleration")
    ...
    def test_csv_has_one_row_per_theme(self):
        ...
        self.assertEqual(
            set(rows[0]),
            {
                "theme", "canonical_repo", "real_data", "primary_source_provenance",
                "reproducible", "scheduled_workflow", "latest_workflow_passed",
                "public_view", "investor2_integration", "evidence_commit",
            },
        )
