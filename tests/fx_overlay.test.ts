...
const observation = (index: number): FxOverlayObservation => {
	...
	return {
		...
		sourceRefs: [`fixture://fx/${day}`, `fixture://swap/${day}`, `fixture://${nextDay}`],
		evidenceKind: "test_fixture",
	};
};
...
describe("FX overlay", () => {
	...
	test("runs PIT walk-forward OOS and keeps fixtures out of VERIFIED output", () => {
		...
		if (result.status !== "TEST_ONLY") throw new Error("expected test-only result");
		...
		expect(result.baselines.find((row) => row.incrementalUsdExposure === 2)?.status).toBe(
			"INFEASIBLE",
		);
	});
	...
	test("fails closed when the requested negative overlay has no realized short swap", () => {
		...
		if (result.status !== "UNVERIFIED") throw new Error("expected unverified result");
