#!/usr/bin/env python3
"""Compare tail-risk allocations on the frozen J-Quants OOS window."""
...
def portfolio_metrics(returns: pd.Series) -> dict[str, float]:
    ...
    volatility = float(np.std(values, ddof=1) * np.sqrt(ANNUALIZATION_FACTOR))
    mean = float(np.mean(values))
    sharpe = float(mean / np.std(values, ddof=1) * np.sqrt(ANNUALIZATION_FACTOR))
    return {
...
def simulate_fold(
    ...
    previous_end_weights: np.ndarray | None,
) -> tuple[pd.Series, np.ndarray, float, float]:
    weights = np.asarray(target_weights, dtype=float).copy()
    if weights.ndim != 1 or weights.shape[0] != test_returns.shape[1]:
        raise AssertionError("target weights do not match test assets")
    ...
    for i, row in enumerate(test_returns.to_numpy(dtype=float)):
        ...
        net = gross - cost if i == 0 else gross
        realized.append(net)
...
def target_weights(train_returns: pd.DataFrame) -> dict[str, np.ndarray]:
    ...
    models = {
        ...
        "variance_risk_budgeting": RiskBudgeting(
            risk_measure=RiskMeasure.VARIANCE,
            min_weights=0.0,
            max_weights=MAX_WEIGHT,
        ),
        "cvar_risk_budgeting": RiskBudgeting(
...
def main() -> None:
    ...
    if len(folds) != 2:
        raise AssertionError("tail-risk study is preregistered for exactly two OOS folds")
    ...
    previous_weights: dict[str, np.ndarray | None] = {name: None for name in STRATEGIES}
    ...
    candidate = metrics["cvar_risk_budgeting"]
    wins = {
        "maximum_drawdown": candidate["maximum_drawdown"] > baseline["maximum_drawdown"],
        "expected_shortfall_95_daily": (
            candidate["expected_shortfall_95_daily"] > baseline["expected_shortfall_95_daily"]
        ),
        "annualized_volatility_within_105pct": (
            candidate["annualized_volatility"] <= baseline["annualized_volatility"] * 1.05
        ),
    ...
    output = {
        "schema_version": "investor2.skfolio-tail-allocation-oos.v1",
        "hypothesis_issue": 259,
