...
def normalize_report(report: dict, report_path: Path) -> dict:
    ...
    if report["empirical_verdict"] == "REPRODUCED":
        report["verdict_reason"] = f"independent full-count training/evaluation matched the predeclared Table 1 beta={beta} tolerances"
    elif report["empirical_verdict"] == "FAILED":
        report["verdict_reason"] = f"independent full-count training/evaluation missed one or more predeclared Table 1 beta={beta} tolerances"
...
def main() -> None:
    ...
    print(json.dumps({"summary": str((output_root / "summary.json").relative_to(ROOT)), "cases": len(summary["cases"])}, sort_keys=True))
