...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Use frozen J-Quants OOS covariance artifacts to compare constrained "
...
def selected_codes_from_summary(summary: dict[str, object]) -> list[str]:
    input_contract = cast(dict[str, object], summary["input_contract"])
    raw_codes = cast(list[object], input_contract["selected_codes"])
...
def evaluate(
    *,
    ...
    summary = cast(
        dict[str, object],
        json.loads((covariance_dir / "summary.json").read_text(encoding="utf-8")),
    )
    folds = cast(list[dict[str, object]], summary["folds"])
    selected_codes = selected_codes_from_summary(summary)
    ...
    return {
        ...
        "verdict": verdict,
        "hypothesis": (
            "true-MktCap Size/Beta covariance improves constrained long-only "
            "minimum-variance OOS risk versus empirical covariance"
        ),
        "acceptance_rule": (
            "USE only if true-MktCap Size/Beta minimum-variance allocation has both "
            "lower after-cost annualized OOS volatility and a smaller-magnitude OOS "
            "maximum drawdown than empirical-covariance minimum-variance allocation; "
            "otherwise REJECT"
        ),
        "allocation_contract": {
        ...
        "claim_boundary": (
            "This evaluates risk allocation over the frozen OOS window only; it does "
            "not establish alpha or expected-return improvement."
        ),
    }
...
def main() -> None:
    ...
    source_summary = cast(
        dict[str, object],
        json.loads((args.covariance_dir / "summary.json").read_text(encoding="utf-8")),
    )
    selected_codes = selected_codes_from_summary(source_summary)
