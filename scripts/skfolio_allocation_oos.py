...
def evaluate(
    *,
    ...
    for fold_record in folds:
        ...
        required = {"baseline_covariance", "current_skfolio_covariance", "candidate_covariance"}
        ...
        for key, weights in weights_by_strategy.items():
            period = pd.Series(test_returns.to_numpy(dtype=float) @ weights, index=test_returns.index, dtype=float)
    ...
    wins = {
        "annualized_volatility": candidate_metrics["annualized_volatility"] < empirical_metrics["annualized_volatility"],
    ...
    return {
        ...
        "claim_boundary": (
            "This evaluates risk allocation over the frozen OOS window only; it does not establish alpha or expected-return improvement."
        ),
    }
...
def main() -> None:
    ...
    selected_codes = [
        str(code) for code in cast(dict[str, object], source_summary["input_contract"])["selected_codes"]
    ]
    ...
    print(json.dumps({"verdict": result["verdict"], "wins": result["primary_metric_wins_vs_empirical"]}, sort_keys=True))
