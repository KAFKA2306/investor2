...
def audit_entry(entry: dict, registry: dict) -> dict:
    ...
    years = [r["fiscalYear"] for r in records]
    ...
    doc_ids = [r["docID"] for r in records]
    ...
    actual_hash = canonical_sha256(records)
    ...
    checks = {
        ...
        "mean_roe_2022_2026": sum(r["roeOfficial"] for r in records) / len(records),
    ...
    for key, actual in checks.items():
        if not close(float(actual), float(derived[key])):
...
def main() -> None:
    ...
    results = [audit_entry(entry, registry) for entry in entries]
    print(json.dumps({"schema_version": "investor2.input-ledger-audit.v1", "status": "PASS", "artifacts": results}, ensure_ascii=False, sort_keys=True))
