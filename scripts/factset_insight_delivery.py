...
class ArticleTextParser(HTMLParser):
    ...
    def text(self) -> str:
        candidates = (self._article_parts, self._main_parts, self._body_parts)
...
def fetch_article(url: str, *, timeout_seconds: int = 60) -> str:
    ...
    with urllib.request.urlopen(request, timeout=timeout_seconds) as response:
        ...
        if final_url.hostname != "insight.factset.com":
    ...
    return extract_article_text(payload)
...
def deliver_pending(
    *,
    ...
    for article in pending:
        ...
        try:
            body = fetch_article(str(article["article_url"]))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Deliver pending FactSet Insight articles after successful local summarization.")
