...
const ReviewConditionSchema = z.union([
  z
    .object({
      type: z.literal("elapsed_days"),
      days: z.number().int().positive(),
    })
    .strict(),
  z
    .object({
      type: z.literal("named_condition"),
...
const AggregatePolicySchema = z
  .object({
    ...
    overlap_handling: z.enum([
      "reject_same_target",
      "allow_distinct_decisions",
    ]),
  })
  .strict()
...
export const DecisionOutcomeSchema = z
  ...
  .superRefine((outcome, ctx) => {
    const expected =
      ...
      outcome.transaction_cost;
    if (Math.abs(expected - outcome.delta) > 1e-9) {
      ctx.addIssue({
        code: "custom",
        path: ["delta"],
        message:
          "delta must equal primary_outcome - benchmark_outcome - transaction_cost",
      });
    }
  });
  ...
  .superRefine((entry, ctx) => {
    ...
    if (decision.decision_id !== snapshot.decision_id) {
      ctx.addIssue({
        ...
        message: "decision must reference snapshot decision_id",
      });
    }
    if (decision.snapshot_ref.commit !== snapshotCommit) {
      ctx.addIssue({
        code: "custom",
        path: ["decision", "snapshot_ref", "commit"],
        message: "decision must reference the exact snapshot commit",
      });
    }
    if (
      plan.decision_id !== snapshot.decision_id ||
      plan.snapshot_ref.commit !== snapshotCommit
    ) {
      ctx.addIssue({
        code: "custom",
        path: ["plan", "snapshot_ref"],
        message: "plan must reference the exact snapshot identity",
      });
    }
    if (decision.outcome_evaluation_plan_ref !== plan.plan_id) {
      ctx.addIssue({
        code: "custom",
        path: ["decision", "outcome_evaluation_plan_ref"],
        message: "decision must reference the attached outcome plan",
      });
    }
    if (Date.parse(plan.fixed_at) > decisionTime) {
      ctx.addIssue({
        code: "custom",
        path: ["plan", "fixed_at"],
        message: "outcome plan must be fixed no later than the human decision",
      });
    }
    if (decisionTime < Date.parse(snapshot.recorded_at)) {
      ctx.addIssue({
        code: "custom",
        path: ["decision", "decided_at"],
        message: "human decision cannot predate its snapshot",
      });
    }
    ...
    if (review.decision_id !== decision.decision_id) {
      ctx.addIssue({
        code: "custom",
        path: ["review", "decision_id"],
        message: "review must reference the human decision",
      });
    }
    if (review.original_snapshot_commit !== snapshotCommit) {
      ctx.addIssue({
        code: "custom",
        path: ["review", "original_snapshot_commit"],
        message: "review must preserve the exact snapshot commit",
      });
    }
    if (review.outcome_evaluation_plan_ref !== plan.plan_id) {
      ctx.addIssue({
        code: "custom",
        path: ["review", "outcome_evaluation_plan_ref"],
        message: "review must use the decision-time outcome plan",
      });
    }
    if (Date.parse(review.outcome.observed_at) < decisionTime) {
      ctx.addIssue({
        code: "custom",
        path: ["review", "outcome", "observed_at"],
        message: "outcome cannot predate the human decision",
      });
    }
    ...
    if (reviewedAt < Date.parse(review.outcome.observed_at)) {
      ctx.addIssue({
        ...
        message: "review cannot predate the outcome observation",
      });
    }
    for (const [index, evidence] of review.outcome_evidence.entries()) {
      ...
      if (availableAt < decisionTime) {
        ctx.addIssue({
          code: "custom",
          path: ["review", "outcome_evidence", index, "available_at"],
          message: "outcome evidence cannot predate the human decision",
        });
      }
      if (availableAt > reviewedAt) {
        ctx.addIssue({
          code: "custom",
          path: ["review", "outcome_evidence", index, "available_at"],
          message: "review cannot use evidence that was not yet available",
        });
      }
    }
  });
...
export const appendHumanDecisionRecord = (
  ...
  const existing = records.find(
    (record) => record.decision_id === parsed.decision_id,
  );
...
const sameAggregatePolicy = (
  left: OutcomeEvaluationPlan,
  right: OutcomeEvaluationPlan,
): boolean => {
...
export const projectDecisionScorecard = (
  ...
  if (policyOwner.aggregate.overlap_handling === "reject_same_target") {
    ...
    for (const entry of actual) {
      ...
      if (targets.has(target)) {
        throw new Error(`overlapping execution target: ${target}`);
      }
      targets.add(target);
    }
  }
  ...
  const totalWeight = resolved.reduce(
    (sum, entry) => sum + entry.plan.aggregate.weight,
    0,
  );
  const weightedDelta =
    totalWeight === 0
      ...
      : resolved.reduce(
          (sum, entry) =>
            sum +
            (entry.review?.outcome.delta ?? 0) * entry.plan.aggregate.weight,
          0,
        ) / totalWeight;
  const weightedDownside =
    totalWeight === 0
      ? null
      : resolved.reduce(
          (sum, entry) =>
            sum +
            (entry.review?.outcome.downside ?? 0) * entry.plan.aggregate.weight,
          0,
        ) / totalWeight;

  let verdict: DecisionScorecard["verdict"] = "INSUFFICIENT_EVIDENCE";
  if (
    resolved.length >= policyOwner.aggregate.minimum_resolved_decisions &&
    weightedDelta !== null
  ) {
