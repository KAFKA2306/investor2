...
function parseInt_(val: string | undefined): number {
	if (!val || val === "－" || val === "") return NaN;
...
function buildRecords(
	...
	for (const [code, stock] of listMap) {
		...
		if (Number.isNaN(finData.eps) || Number.isNaN(finData.bps)) continue;
		if (Number.isNaN(finData.equity) || Number.isNaN(finData.totalAssets))
			continue;

		const shares = Number.isNaN(finData.shares) ? 0 : finData.shares;
		...
		const profit = Number.isNaN(finData.profit) ? 0 : finData.profit;
		const roe = finData.equity > 0 ? (profit / finData.equity) * 100 : NaN;
		const roa =
			finData.totalAssets > 0 ? (profit / finData.totalAssets) * 100 : NaN;

		const opProfit = Number.isNaN(finData.operatingProfit)
			? 0
			: finData.operatingProfit;
		const netSales = Number.isNaN(finData.netSales) ? 0 : finData.netSales;
